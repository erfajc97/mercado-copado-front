...
export function FormPaymentMethod({
  ...
  const handleFormFinish = async (values: any) => {
    if (transformFormData) {
      ...
      // Si no se proporciona transformFormData, usar helper directamente
      const { transformPaymentMethodData } = await import(
        '../helpers/transformPaymentMethodData'
      )
      const transformedData = transformPaymentMethodData(values)
      await onFinish(transformedData)
    }
  }
